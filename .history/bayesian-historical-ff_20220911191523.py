...
def simulate(team, db, years, exps=10):
    ...
    with alive_bar(exps, bar='classic') as bar:
        for n in range(exps):
...
# player level stats
outcome.to_csv("results/{current_year}_projections_from_years{years}.csv".format(current_year = todays_date.year, years = my_years), sep = "\t")
