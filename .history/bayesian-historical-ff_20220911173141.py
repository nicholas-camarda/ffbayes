...
def get_score(p, db, year, week):
    return(db[(db["Name"] == p) & (db["Season"] == year) & (db["G#"] == week)])
...
def get_score_for_player(db, player, years):
    ...
    for p in get_games(db, year,week).itertuples():
        if player == p.Name:
            return get_score(p, db, year, week)
        
    # return get_score_for_player(player) # Retry due to bye weeks / failure for any other reason
get_score_for_player(db = combined_data, player = my_team.loc[0, "Name"], years = my_years)

print(combined_data.loc[0, ["FantPt"]])
