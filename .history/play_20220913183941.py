...
with open(r"model.pickle", "rb") as input_file:
    ...
    proj_mae = mean_absolute_error(y_true = test.loc[:, "7_game_avg"], y_pred = ppc["FantPt"], multioutput='raw_values')
    ...
    print(len(proj_mae), len(h_proj_mae), len(proj_sd))
    d = pd.DataFrame({"proj MAE": proj_mae,
                      "historical avg MAE": h_proj_mae,
                      "sd": proj_sd})
