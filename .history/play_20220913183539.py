...
with open(r"model.pickle", "rb") as input_file:
    ...
    d = pd.DataFrame({"proj MAE": mean_absolute_error(y_true = test[:, "7_game_avg"], y_pred = ppc["FantPt"].mean(), multioutput='raw_values'),
                      "historical avg MAE": mean_absolute_error(y_true = test[:, "FantPt"].mean(), y_pred = test[:, "7_game_avg"].mean(), multioutput='raw_values'),
                      "sd": test[:, "7_game_avg"].std(axis = 0)})
