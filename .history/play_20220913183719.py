...
with open(r"model.pickle", "rb") as input_file:
    ...
    d = pd.DataFrame({"proj MAE": mean_absolute_error(y_true = test.loc[:, "7_game_avg"], y_pred = ppc["FantPt"].mean(axis=0), multioutput='raw_values'),
                      "historical avg MAE": mean_absolute_error(y_true = test.loc[:, "FantPt"], y_pred = test.loc[:, "7_game_avg"], multioutput='raw_values'),
                      "sd": ppc["FantPt"].std(axis = 0)})
