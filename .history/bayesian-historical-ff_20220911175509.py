...
def score_player(p, db, year, week):
    sc = db[(db["Name"] == p.Name) & (db["Season"] == year) & (db["G#"] == week)]
    return(sc)
...
def get_score_for_player(db, player, years):
    ...
    for p in get_games(db, year, week).itertuples():
        if p.Name is None:
            continue
        if player == p.Name:
            return score_player(p, db, year, week)
        
    # return get_score_for_player(db, player, years) # Retry due to bye weeks / failure for any other reason


# score1 = get_score_for_player(db = combined_data, player = my_team.loc[0, "Name"], years = my_years)

def simulate(team, db, years, exps=10):
    scores = pd.DataFrame(data=np.zeros((exps,len(team.drop_duplicates()))),
                          columns = [p.Name for p in team.drop_duplicates().itertuples()])
    ...
    for n in range(exps):
        for player in team.itertuples():
            scores.loc[n, player.Name] += get_score_for_player(db, player, years)
    return scores
