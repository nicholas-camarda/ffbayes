...
def simulate(team, db, years, exps=10):
    ...
    bar = progressbar.ProgressBar(max_value=exps)
    for n in progressbar.progressbar(range(exps)):
...
output_file = "results/{years}_projections.csv".format(years = my_years)
my_dirname = os.path.dirname(output_file)
print(my_dirname)
os.makedirs(my_dirname, exist_ok=True)
outcome.mean().write_csv(output_file)