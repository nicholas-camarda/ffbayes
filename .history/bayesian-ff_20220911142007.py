...
def validate_team(team, my_team):
    ...
    for t in unique_teams.to_records():
        print(t.Name, t.Position, t.Tm)
    if len(team) != len(my_team):
        pd_my_team = pd.DataFrame(my_team)
        res = unique_teams[~unique_teams.apply(tuple,1).isin(pd_my_team.apply(tuple,1))]
        print("Dfs are not the same. Missing:")
        print(res)
