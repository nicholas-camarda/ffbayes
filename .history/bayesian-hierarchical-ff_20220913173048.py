...
def bayesian_hierarchical_ff(cores):
    ...
    with pm.Model() as mdl:
        ...
        # DEBUG: what is 'd' ?????
        d = pd.DataFrame({'proj MAE':  mean_absolute_error(test.loc[:,'FantPt'].values, ppc['FantPt'], multioutput='rawvalues'), 
                        'historical av MAE': tr['defensive differential rb'].mean(axis=0),
