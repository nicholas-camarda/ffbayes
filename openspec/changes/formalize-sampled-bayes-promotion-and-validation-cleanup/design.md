...
2. keep empirical Bayes as production and retain sampled follow-up only for prior-design learning
3. keep empirical Bayes as production and retire the sampled lane from the supported workflow

The intermediate comparison artifact may still record `keep_empirical_bayes`, but the final interpretation report SHALL resolve that non-promotion result into either prior follow-up or retirement before the repository is considered done with this change.
