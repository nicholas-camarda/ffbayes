...
1. `promote_sampled_bayes`
2. `keep_empirical_bayes_with_sampled_prior_followup`
3. `keep_empirical_bayes_and_retire_sampled_workflow`

The earlier comparison artifact may record the intermediate non-promotion outcome `keep_empirical_bayes`, but the final interpretation artifact MUST resolve that state into either prior follow-up or retirement before the repository move-forward decision is complete.
