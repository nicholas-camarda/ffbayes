...
The guide suite MUST use one authoritative technical and methods guide to describe the implemented draft board pipeline, forecast target, player-model architecture, rookie-prior structure, validation scope, decision-policy layer, baselines, and interval semantics in terms that match the current code and emitted artifacts. The guide MUST remove retired hybrid-path descriptions from the supported workflow.
...
- **WHEN** the authoritative technical guide describes uncertainty, floor and ceiling values, confidence intervals, or evidence summaries
...
#### Scenario: Technical guide names retired hybrid path as removed
- **WHEN** the authoritative technical guide discusses the supported pre-draft workflow
- **THEN** it MUST NOT describe the hybrid Monte Carlo plus random-forest path as an active or compatibility workflow for the current draft engine
...
#### Scenario: Lay guide uses concrete player examples
- **WHEN** the layperson guide explains uncertainty, rookie priors, or availability risk
- **THEN** it MUST include concrete examples such as a durable mid-tier quarterback, a team-changing veteran receiver, or a rookie with strong draft capital but uncertain depth-chart role
...
- **THEN** it MUST describe the flow from collected season files through preprocessing, unified dataset construction, player-forecast generation, draft board artifact creation, and optional publish or retrospective steps
