...
def bayesian_hierarchical_ff(path_to_data_directory, cores):
    # We are using a single year for the analysis
    data, team_names = create_dataset(path_to_data_directory)
    # explore = data[
    #     data.apply(lambda x: x["Season"] == ["2017", "2018", "2019"], axis=1)
    # ]
    ...
    with pm.Model() as mdl:
        ...
        nu = (
            pm.Exponential('nu minus one', 1 / 29.0, shape=2) + 1
        )  # from https://pymc-devs.github.io/pymc3/notebooks/BEST.html
        ...
        player_avg = theano.shared(
            np.asarray((train['7_game_avg']).values, dtype=float),
        )
        ...
        qb = theano.shared(
            np.asarray((train['position_QB']).values.astype(int), dtype=int),
        )
        wr = theano.shared(
            np.asarray((train['position_WR']).values.astype(int), dtype=int),
        )
        rb = theano.shared(
            np.asarray((train['position_RB']).values.astype(int), dtype=int),
        )
        te = theano.shared(
            np.asarray((train['position_TE']).values.astype(int), dtype=int),
        )
        pos_id = theano.shared(np.asarray((train['pos_id']).values, dtype=int))

        print(
            'Part 3... Defensive ability of the opposing team vs. each position, partially pooled',
        )
        ...
        opp_qb = pm.Normal(
            'defensive differential qb',
            opp_def[0],
            sd=100**2,
            shape=team_number,
        )
        opp_wr = pm.Normal(
            'defensive differential wr',
            opp_def[1],
            sd=100**2,
            shape=team_number,
        )
        opp_rb = pm.Normal(
            'defensive differential rb',
            opp_def[2],
            sd=100**2,
            shape=team_number,
        )
        opp_te = pm.Normal(
            'defensive differential te',
            opp_def[3],
            sd=100**2,
            shape=team_number,
        )
        ...
        print(
            "Part 4... Partially pooled ability of the player's rank partially pooled based on position",
        )
        ...
        pos_home_qb = pm.Normal(
            'home differential qb',
            home_adv[0],
            10**2,
            shape=ranks,
        )
        pos_home_rb = pm.Normal(
            'home differential rb',
            home_adv[1],
            10**2,
            shape=ranks,
        )
        pos_home_te = pm.Normal(
            'home differential te',
            home_adv[2],
            10**2,
            shape=ranks,
        )
        pos_home_wr = pm.Normal(
            'home differential wr',
            home_adv[3],
            10**2,
            shape=ranks,
        )
        pos_away_qb = pm.Normal(
            'away differential qb',
            away_adv[0],
            10**2,
            shape=ranks,
        )
        pos_away_rb = pm.Normal(
            'away differential rb',
            away_adv[1],
            10**2,
            shape=ranks,
        )
        pos_away_wr = pm.Normal(
            'away differential wr',
            away_adv[2],
            10**2,
            shape=ranks,
        )
        pos_away_te = pm.Normal(
            'away differential te',
            away_adv[3],
            10**2,
            shape=ranks,
        )
        ...
        print(
            "Part 5...  First likelihood where the player's difference from average is explained by defensive abililty",
        )
        # First likelihood where the player's difference from average is explained by defensive abililty
        def_effect = (
            qb * opp_qb[player_opp]
            + wr * opp_wr[player_opp]
            + rb * opp_rb[player_opp]
            + te * opp_te[player_opp]
        )
        like1 = pm.StudentT(
            'Diff From Avg',
            mu=def_effect,
            sd=err_b[player_rank],
            nu=nu[1],
            observed=train['diff_from_avg'],
        )

        print(
            'Part 6... Second likelihood where the score is predicted by defensive power plus other smaller factors',
        )
        # Second likelihood where the score is predicted by defensive power plus other smaller factors
        mu = player_avg + def_effect
        mu += rb * pos_home_rb[player_rank] * (player_home) + wr * pos_home_wr[player_rank] * (
            player_home
        )
        mu += qb * pos_home_qb[player_rank] * (player_home) + te * pos_home_te[player_rank] * (
            player_home
        )
        mu += rb * pos_away_rb[player_rank] * (1 - player_home) + wr * pos_away_wr[player_rank] * (
            1 - player_home
        )
        mu += qb * pos_away_qb[player_rank] * (1 - player_home) + te * pos_away_te[player_rank] * (
            1 - player_home
        )
        like2 = pm.StudentT(
            'FantPt',
        ...
        print(
            ...
            mean_absolute_error(
                test.loc[:, 'FantPt'].values,
                ppc['FantPt'].mean(axis=0),
            ),
        )
        print(
            '7 Day Average Mean Absolute Error:',
            mean_absolute_error(
                test.loc[:, 'FantPt'].values,
                test.loc[:, '7_game_avg'].values,
            ),
        )
        ...
        ax = plt.gca()
        ax.plot(
            np.linspace(0, max_sd, 30),
            np.array(
                [d[d['sd'] <= k]['proj MAE'].mean() for k in np.linspace(0, max_sd, 30)],
            ),
        )
        ax.plot(
            np.linspace(0, max_sd, 30),
            np.array(
                [d[d['sd'] <= k]['historical avg MAE'].mean() for k in np.linspace(0, max_sd, 30)],
            ),
            color='r',
        )
        ...
        fig = plt.gcf()  # to get the current figure...
        fig.savefig(
            'plots/MAE_for_projections_w_sd_under_cutoff.png',
            dpi=300,
        ...
        t = pd.DataFrame(
            {
                'projection': tr['defensive differential rb'].mean(axis=0),
        ...
        t = pd.DataFrame(
            {
                'projection': tr['defensive differential qb'].mean(axis=0),
                'sd': tr['defensive differential qb'].std(axis=0),
                'name': team_names,
            },
        )
        f = plt.figure(figsize=(8, 10))
        ...
        t = pd.DataFrame(
            {
                'projection': tr['defensive differential te'].mean(axis=0),
                'sd': tr['defensive differential te'].std(axis=0),
                'name': team_names,
            },
        )
        f = plt.figure(figsize=(8, 10))
        ...
        t = pd.DataFrame(
            {
                'projection': tr['defensive differential wr'].mean(axis=0),
                'sd': tr['defensive differential wr'].std(axis=0),
                'name': team_names,
            },
        )
        f = plt.figure(figsize=(8, 10))
