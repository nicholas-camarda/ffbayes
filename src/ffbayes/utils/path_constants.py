...
def get_results_dir(year: int = None, phase: str | None = None) -> Path:
    ...
    path = get_run_root(year, phase) / 'results'
...
def get_dashboard_dir(year: int = None, phase: str | None = None) -> Path:
    ...
    path = get_run_root(year, phase) / 'dashboard'
    ensure_dir_exists(path)
    return path


def get_pre_draft_dashboard_dir(year: int = None) -> Path:
    """Get pre-draft dashboard directory."""
    path = get_dashboard_dir(year, 'pre_draft')
    ensure_dir_exists(path)
    return path
...
def get_dashboard_payload_path(year: int = None) -> Path:
    ...
    if year is None:
        ...
        year = datetime.now().year
    return get_pre_draft_dashboard_dir(year) / f'dashboard_payload_{year}.json'
...
def get_dashboard_html_path(year: int = None) -> Path:
    ...
    if year is None:
        ...
        year = datetime.now().year
    return get_pre_draft_dashboard_dir(year) / f'draft_board_{year}.html'
...
def get_plots_dir(year: int = None) -> Path:
    ...
    if year is None:
        ...
        year = datetime.now().year
    path = get_run_root(year, get_phase_name()) / 'plots'
    ensure_dir_exists(path)
    return path
...
def get_pre_draft_plots_dir(year: int = None) -> Path:
    ...
    path = get_run_root(year, 'pre_draft') / 'plots'
    ensure_dir_exists(path)
    return path
...
def create_all_required_directories(year: int = None) -> None:
    ...
    ensure_dir_exists(get_post_draft_dashboard_dir(year))
