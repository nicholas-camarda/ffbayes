...
class EnhancedPipelineOrchestrator:
    ...
    def _validate_configuration(self, config: Dict[str, Any]) -> None:
        ...
        for step in config['pipeline_steps']:
            ...
            for key in required_step_keys:
                if key not in step:
                    raise PipelineError(
                        f"Step '{step.get('name', 'unknown')}' missing required key: {key}"
                    )
            ...
            if step['retry_count'] < 0:
                raise PipelineError(
                    f"Step '{step['name']}' must have non-negative retry count"
                )
    ...
    def _calculate_execution_order(self) -> List[str]:
        ...
        if self._has_circular_dependencies(dependency_graph):
            raise PipelineError(
                'Circular dependencies detected in pipeline configuration'
            )
    ...
    def _can_execute_parallel(
        self, step: PipelineStep, running_steps: List[PipelineStep]
    ) -> bool:
        ...
        running_in_group = sum(
            1 for s in running_steps if s.parallel_group == step.parallel_group
        )
        ...
        try:
            ...
            if module_name:
                cmd = [sys.executable, '-m', module_name] + (
                    shlex.split(step.args) if step.args else []
                )
            else:
            ...
            if return_code == 0:
                ...
                logger.info(
                    f'Step {step.name} completed successfully in {step.execution_time:.1f}s'
                )
                return PipelineResult(
                    step_name=step.name,
                    success=True,
    ...
    def _should_retry_step(self, step: PipelineStep, result: PipelineResult) -> bool:
        ...
        if (
            self.error_recovery_state['total_retries']
            >= self.config['error_handling']['max_total_retries']
        ):
            return False
    ...
    def _retry_step(self, step: PipelineStep) -> PipelineResult:
        ...
        if self.config['error_handling']['retry_strategies']['exponential_backoff']:
            backoff_multiplier = min(
                step.retry_attempts,
                self.config['error_handling']['retry_strategies'][
        ...
        logger.info(
            f'Retrying step {step.name} in {retry_delay:.1f}s (attempt {step.retry_attempts})'
        )
    ...
    def _handle_step_failure(self, step: PipelineStep, result: PipelineResult) -> bool:
        ...
        if step.critical:
            ...
            if (
                self.config['error_handling']['critical_failure_action']
                == 'stop_pipeline'
            ):
        ...
        if step.critical:
            logger.error(
                f'Critical step {step.name} failed after retries. Pipeline cannot continue.'
            )
            return False

        # Non-critical step failed - check graceful degradation
        if (
            self.config['error_handling']['non_critical_failure_action']
            == 'continue_with_warnings'
        ):
            logger.warning(
                f'Non-critical step {step.name} failed. Continuing with warnings.'
            )
            step.status = StepStatus.FAILED
            return True
        ...
        try:
            while len(completed_steps) < len(self.steps):
                ...
                if self.config['monitoring']['progress_update_interval'] > 0:
                    if (
                        int(time.time() - self.pipeline_start_time)
                        % self.config['monitoring']['progress_update_interval']
                        == 0
                    ):
            ...
            critical_failures = any(
                step.status == StepStatus.FAILED and step.critical
                for step in self.steps
            )
    ...
    def _execute_step_thread(
        ...
        step: PipelineStep,
        completed_steps: set,
        running_steps: List[PipelineStep],
    ) -> None:
    ...
    def _print_progress(self, completed_steps: set) -> None:
        ...
        logger.info(
            f'Pipeline Progress: {completed_count}/{total_steps} steps completed ({progress_percent:.1f}%) - {eta_str}'
        )
    ...
    def _finalize_pipeline(self) -> None:
        ...
        if self.performance_metrics['completed_steps'] > 0:
            theoretical_sequential_time = sum(
                result.execution_time for result in self.results
            )
        ...
        logger.info(
            f'Parallel efficiency: {self.performance_metrics["parallel_efficiency"]:.2f}'
        )
