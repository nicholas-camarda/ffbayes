...
def _copy_tree(
    ...
    *,
    ...
    skip_glob_list = list(skip_globs)

    for file_path in sorted(source_dir.rglob('*')):
        if not file_path.is_file():
            continue
        if file_path.name in skip_name_set:
            continue
        relative_path = file_path.relative_to(source_dir)
        if any(relative_path.match(pattern) for pattern in skip_glob_list):
            continue
...
def _sync_stable_data(current_year: int) -> dict[str, list[str]]:
    ...
    return {
        'raw_data_files': copied_raw,
        'processed_data_files': copied_processed,
    }
...
def _publish_analysis_snapshot(current_year: int) -> dict[str, Any]:
    ...
    if dashboard_html.exists():
        copied_dashboard.append(_copy_file(dashboard_html, dashboard_dir / 'index.html'))
    ...
    manifest = {
        'schema_version': 'cloud_publish_manifest_v2',
    ...
    return {
        ...
        'dashboard_files': copied_dashboard,
        'analysis_files': copied_analysis,
        'copied_snapshot_files': copied_dashboard + copied_analysis + [str(manifest_path)],
    }
