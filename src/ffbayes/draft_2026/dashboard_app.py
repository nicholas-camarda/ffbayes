...
function renderBoard(){ const body=$('board'); body.innerHTML=''; if(!currentPayload)return; for(const row of currentPayload.decision_table.slice(0,100)){ const tr=document.createElement('tr'); tr.dataset.playerId=String(row.espn_id); const status=row.roster_status||'available'; tr.innerHTML=`<td>${row.board_rank}</td><td>${row.name}</td><td>${row.position}</td><td>${Number(row.projected_points).toFixed(1)}</td><td>${Number(row.replacement_level).toFixed(1)}</td><td>${Number(row.vor).toFixed(1)}</td><td>${Number(row.scarcity).toFixed(1)}</td><td>${Number(row.adp).toFixed(1)}</td><td>${row.availability_next_pick==null?'Draft slot required':(Number(row.availability_next_pick)*100).toFixed(0)+'%'}</td><td><button data-action="taken" data-id="${row.espn_id}">Taken</button> <button data-action="mine" data-id="${row.espn_id}">Mine</button> <button data-action="queue" data-id="${row.espn_id}">Queue</button></td><td class="status-${status}">${status} · ${row.recommendation}</td>`; body.appendChild(tr); } $('status').textContent=`Current pick ${currentPayload.current_pick ?? '—'} · next pick ${currentPayload.next_pick ?? '—'} · ${currentPayload.decision_table.length} validated players`; $('provenance').textContent=JSON.stringify(currentPayload.provenance,null,2); }
