...
# Configuration
DEFAULT_CORES = 7
DEFAULT_DRAWS = 2000  # Increased for production reliability
DEFAULT_TUNE = 2000   # Increased for production reliability
DEFAULT_CHAINS = 4
DEFAULT_PREDICTIVE_SAMPLES = 1000  # Increased for better predictions
...
if QUICK_TEST:
    ...
    print("⚠️  WARNING: This will use MINIMAL parameters and UNRELIABLE metrics")
    DEFAULT_CORES = 1
    DEFAULT_DRAWS = 10
    DEFAULT_TUNE = 10
    DEFAULT_CHAINS = 1
...
def merge_unified_features(base_data):
    ...
    # Calculate advanced stats from existing data
    print("Calculating advanced stats from existing fantasy football data...")
    base_data = calculate_advanced_stats_from_existing_data(base_data)
    print(f"Advanced stats calculation complete. Data shape: {base_data.shape}")
    
    # Note: Weather data integration is not implemented yet
    # This would require OpenWeatherMap API integration or ESPN API weather data
    # For now, we'll proceed without weather data to test the advanced stats integration
    print("Weather data integration not implemented - proceeding without weather features")
    
    # Note: Vegas odds data integration is not implemented yet
    # This would require ESPN API, SportsData.io, or Odds API integration
    # For now, we'll proceed without odds data to test the advanced stats integration
    print("Vegas odds data integration not implemented - proceeding without odds features")
    ...
    # Merge snap counts
    # This section is removed as per the edit hint to remove unimplemented functions
...
def unified_bayesian_model(path_to_data_directory, cores=DEFAULT_CORES, draws=DEFAULT_DRAWS, tune=DEFAULT_TUNE, chains=DEFAULT_CHAINS, predictive_samples=DEFAULT_PREDICTIVE_SAMPLES):
    ...
    try:
        ...
        data = load_unified_dataset(path_to_data_directory)
        print(f"✅ Loaded unified dataset: {data.shape}")
        print(f"✅ Available years: {sorted(data['Season'].unique())}")
        
    except Exception as e:
        print(f"❌ Error loading unified dataset: {str(e)}")
        raise

    # Downsample in QUICK_TEST to ensure speed while keeping train/test split possible
    if os.getenv('QUICK_TEST', 'false').lower() == 'true':
        print("⚠️  QUICK_TEST mode: Downsampling data for speed...")
        
        # Instead of random downsampling, ensure we get the players we need
        # Load test team to know which players to prioritize
        from ffbayes.utils.path_constants import get_default_team_file
        test_team_path = str(get_default_team_file())
        priority_players = []
        
        try:
            if os.path.exists(test_team_path):
                test_team_df = pd.read_csv(test_team_path, sep='\t')
                priority_players = test_team_df['Name'].tolist()
                print(f"📋 Found {len(priority_players)} priority players from test team")
        except:
            print("⚠️  Could not load test team, using random downsampling")
        
        if priority_players:
            # Get all data for priority players first
            priority_data = data[data['Name'].isin(priority_players)]
            print(f"✅ Priority players data: {priority_data.shape}")
            
            # Get remaining data to fill up to target size
            remaining_data = data[~data['Name'].isin(priority_players)]
            target_remaining = 400 - len(priority_data)
            
            if target_remaining > 0:
                # Sample remaining data to get to target size
                remaining_sample = remaining_data.sample(n=min(target_remaining, len(remaining_data)), random_state=42)
                data = pd.concat([priority_data, remaining_sample], ignore_index=True)
                print(f"✅ Combined priority + sample data: {data.shape}")
            else:
                data = priority_data
                print(f"✅ Using only priority players data: {data.shape}")
        else:
            # Fallback to original random downsampling
            data = data.sample(n=400, random_state=42)
            print(f"⚠️  Random downsampling to {data.shape}")
        
        print(f"Years in downsampled data: {sorted(data['Season'].unique())}")
    ...
    # Split into train/test - use actual test year instead of automatic split
    ...
    # Use the actual test year we want (2025) instead of automatic split
    target_test_year = datetime.now().year
    if target_test_year in available_years:
        # If 2025 data exists, use it for testing
        train_year = available_years[-2]  # Use second-to-last year for training
        test_year = target_test_year
    else:
        # Fallback to automatic split if target year not available
        train_year = available_years[-2]
        test_year = available_years[-1]
        print(f"⚠️  Target test year {target_test_year} not available, using {test_year}")
    ...
    print(f"🎯 Target test year: {target_test_year}")
    
    # Clean up data types and handle NaN values for PyMC compatibility
    print("🧹 Cleaning data types and handling NaN values...")
    
    # Create proper copies to avoid SettingWithCopyWarning
    train = train.copy()
    test = test.copy()
    
    # Ensure numeric columns have proper types and no NaN values
    numeric_columns = ['7_game_avg', 'FantPt', 'consistency_score', 'boom_bust_ratio', 
                      'team_usage_pct', 'home_away_diff', 'recent_form', 'season_trend',
                      'defense_qb_mean', 'defense_rb_mean', 'defense_wr_mean', 'defense_te_mean',
                      'week_early', 'week_mid', 'week_late', 'season_progress', 'is_rookie', 'is_veteran']
    
    for col in numeric_columns:
        if col in train.columns:
            # Convert to numeric, replacing any non-numeric values with 0
            train.loc[:, col] = pd.to_numeric(train[col], errors='coerce').fillna(0)
            test.loc[:, col] = pd.to_numeric(test[col], errors='coerce').fillna(0)
            print(f"   ✅ Cleaned {col}: {train[col].dtype}")
    
    # Simple fix for is_home column - just convert True->1, False/NaN->0
    ...
    try:
        with pm.Model() as unified_model:
            ...
            # Create is_home indicator directly from Position and Opp columns
            # If player's team matches home_team, they're home
            player_home = pm.Data('player_home', (train['Position'] == 'DEF').astype(int))  # Placeholder - all 0 for now
            player_avg = pm.Data('player_avg', train['7_game_avg'].values)
            # Create player_rank with bounds checking
            if 'rank' in train.columns:
                raw_ranks = train['rank'].values - 1
                # Ensure ranks are within bounds [0, ranks-1]
                bounded_ranks = np.clip(raw_ranks, 0, ranks - 1).astype(int)
                player_rank = pm.Data('player_rank', bounded_ranks)
                print(f"   ✅ Player ranks: {bounded_ranks.min()}-{bounded_ranks.max()}")
            else:
                player_rank = pm.Data('player_rank', np.zeros(len(train), dtype=int))
                print("   ✅ Using default player ranks (all 0)")
            # Create position indicators from the Position column
            qb_indicator = pm.Data('qb_indicator', (train['Position'] == 'QB').astype(int))
            wr_indicator = pm.Data('wr_indicator', (train['Position'] == 'WR').astype(int))
            rb_indicator = pm.Data('rb_indicator', (train['Position'] == 'RB').astype(int))
            te_indicator = pm.Data('te_indicator', (train['Position'] == 'TE').astype(int))
            ...
            consistency_score = pm.Data('consistency_score', train['consistency_score'].values if 'consistency_score' in train.columns else np.zeros(len(train)))
            boom_bust_ratio = pm.Data('boom_bust_ratio', train['boom_bust_ratio'].values if 'boom_bust_ratio' in train.columns else np.zeros(len(train)))
            team_usage_pct = pm.Data('team_usage_pct', train['team_usage_pct'].values if 'team_usage_pct' in train.columns else np.zeros(len(train)))
            home_away_diff = pm.Data('home_away_diff', train['home_away_diff'].values if 'home_away_diff' in train.columns else np.zeros(len(train)))
            recent_form = pm.Data('recent_form', train['recent_form'].values if 'recent_form' in train.columns else np.zeros(len(train)))
            season_trend = pm.Data('season_trend', train['season_trend'].values if 'season_trend' in train.columns else np.zeros(len(train)))
            ...
            # Advanced stats effect priors
            consistency_effect = pm.Normal('consistency_effect', 0, 0.5)
            boom_bust_effect = pm.Normal('boom_bust_effect', 0, 0.5)
            team_usage_effect = pm.Normal('team_usage_effect', 0, 0.5)
            home_away_effect = pm.Normal('home_away_effect', 0, 0.5)
            recent_form_effect = pm.Normal('recent_form_effect', 0, 0.5)
            season_trend_advanced_effect = pm.Normal('season_trend_advanced_effect', 0, 0.5)
            ...
            # Add advanced stats effects
            advanced_stats_effects = (
                consistency_effect * consistency_score +
                boom_bust_effect * boom_bust_ratio +
                team_usage_effect * team_usage_pct +
                home_away_effect * home_away_diff +
                recent_form_effect * recent_form +
                season_trend_advanced_effect * season_trend
            )
            mu += advanced_stats_effects
            ...
            if chains == 1:
                init_strategy = 'auto'  # Use auto initialization for single chain
            else:
            ...
            trace = pm.sample(
                ...
                init=init_strategy,  # Correct parameter name for initialization strategy
            ...
            trace_file = str(get_results_dir(current_year) / 'bayesian-hierarchical-results' / f'unified_trace_{current_year}.pkl')
            ...
            with unified_model:
                ...
                test_data_dict = {
                    ...
                    'wr_indicator': (test['Position'] == 'WR').astype(int),
                    'rb_indicator': (test['Position'] == 'RB').astype(int),
                    'te_indicator': (test['Position'] == 'TE').astype(int),
            ...
            # Save individual player predictions for team aggregation and draft strategy
            player_predictions = {}
            
            # Determine which players to predict for based on mode
            if QUICK_TEST:
                # In QUICK_TEST mode, try to load test team for focused testing
                from ffbayes.utils.path_constants import get_default_team_file
                test_team_path = str(get_default_team_file())
                target_players = []
                ...
                try:
                    if os.path.exists(test_team_path):
                        test_team_df = pd.read_csv(test_team_path, sep='\t')
                        target_players = test_team_df['Name'].tolist()
                        print(f"🎯 QUICK_TEST mode: Predicting for test team players: {len(target_players)} players")
                    else:
                        print("⚠️  Test team file not found in QUICK_TEST mode, using test dataset players")
                        target_players = test['Name'].values.tolist()
                except Exception as e:
                    print(f"⚠️  Error loading test team in QUICK_TEST mode: {e}, using test dataset players")
                    target_players = test['Name'].values.tolist()
            else:
                # In production mode, predict for all test dataset players
                target_players = test['Name'].values.tolist()
                print(f"🎯 Production mode: Predicting for all test dataset players: {len(target_players)} players")
            
            # Make predictions for target players
            for player_name in target_players:
                # Check if player exists in test data (2024) first
                if player_name in test['Name'].values:
                    # Player exists in test data - use actual prediction
                    player_idx = test[test['Name'] == player_name].index[0]
                    player_predictions[str(player_name)] = {
                        'mean': float(pred_mean.values[player_idx]),
                        'std': float(pred_std.values[player_idx]),
                        'confidence_interval': [float(pred_5th.values[player_idx]), float(pred_95th.values[player_idx])],
                        'percentiles': {
                            'p5': float(pred_5th.values[player_idx]),
                            'p25': float(pred_25th.values[player_idx]),
                            'p75': float(pred_75th.values[player_idx]),
                            'p95': float(pred_95th.values[player_idx])
                        },
                        'position': str(test.loc[player_idx, 'Position']),
                        'team': str(test.loc[player_idx, 'Tm']) if 'Tm' in test.columns else None,
                        'advanced_stats': {
                            'consistency_score': float(test.loc[player_idx, 'consistency_score']) if 'consistency_score' in test.columns and pd.notna(test.loc[player_idx, 'consistency_score']) else None,
                            'boom_bust_ratio': float(test.loc[player_idx, 'boom_bust_ratio']) if 'boom_bust_ratio' in test.columns and pd.notna(test.loc[player_idx, 'boom_bust_ratio']) else None,
                            'team_usage_pct': float(test.loc[player_idx, 'team_usage_pct']) if 'team_usage_pct' in test.columns and pd.notna(test.loc[player_idx, 'team_usage_pct']) else None,
                            'home_away_diff': float(test.loc[player_idx, 'home_away_diff']) if 'home_away_diff' in test.columns and pd.notna(test.loc[player_idx, 'home_away_diff']) else None,
                            'recent_form': float(test.loc[player_idx, 'recent_form']) if 'recent_form' in test.columns and pd.notna(test.loc[player_idx, 'recent_form']) else None,
                            'season_trend': float(test.loc[player_idx, 'season_trend']) if 'season_trend' in test.columns and pd.notna(test.loc[player_idx, 'season_trend']) else None
                        }
                    }
                elif player_name in data['Name'].values:
                    # Player exists in historical data but not in 2024 test data
                    # Use historical data to create informed prediction
                    print(f"📊 Player {player_name} found in historical data, creating informed prediction")
                    player_data = data[data['Name'] == player_name]
                    
                    # Get most recent season data for this player
                    latest_season = player_data['Season'].max()
                    latest_data = player_data[player_data['Season'] == latest_season]
                    
                    if len(latest_data) > 0:
                        # Use historical performance to create prediction
                        historical_mean = latest_data['FantPt'].mean()
                        historical_std = latest_data['FantPt'].std()
                        
                        # Create prediction based on historical performance
                        player_predictions[str(player_name)] = {
                            'mean': float(historical_mean),
                            'std': float(historical_std),
                            'confidence_interval': [float(historical_mean - historical_std), float(historical_mean + historical_std)],
                            'percentiles': {
                                'p5': float(historical_mean - 1.645 * historical_std),
                                'p25': float(historical_mean - 0.675 * historical_std),
                                'p75': float(historical_mean + 0.675 * historical_std),
                                'p95': float(historical_mean + 1.645 * historical_std)
                            },
                            'position': str(latest_data['Position'].iloc[0]),
                            'team': str(latest_data['Tm'].iloc[0]) if 'Tm' in latest_data.columns else None,
                            'advanced_stats': {
                                'consistency_score': float(latest_data['consistency_score'].iloc[0]) if 'consistency_score' in latest_data.columns and pd.notna(latest_data['consistency_score'].iloc[0]) else None,
                                'boom_bust_ratio': float(latest_data['boom_bust_ratio'].iloc[0]) if 'boom_bust_ratio' in latest_data.columns and pd.notna(latest_data['boom_bust_ratio'].iloc[0]) else None,
                                'team_usage_pct': float(latest_data['team_usage_pct'].iloc[0]) if 'team_usage_pct' in latest_data.columns and pd.notna(latest_data['team_usage_pct'].iloc[0]) else None,
                                'home_away_diff': float(latest_data['home_away_diff'].iloc[0]) if 'home_away_diff' in latest_data.columns and pd.notna(latest_data['home_away_diff'].iloc[0]) else None,
                                'recent_form': float(latest_data['recent_form'].iloc[0]) if 'recent_form' in latest_data.columns and pd.notna(latest_data['recent_form'].iloc[0]) else None,
                                'season_trend': float(latest_data['season_trend'].iloc[0]) if 'season_trend' in latest_data.columns and pd.notna(latest_data['season_trend'].iloc[0]) else None
                            },
                            'note': f'Historical prediction based on {latest_season} season data'
                        }
                    else:
                        # Fallback to baseline prediction
                        print(f"⚠️  Player {player_name} has no valid historical data, using baseline")
                        player_predictions[str(player_name)] = {
                            'mean': 15.0,  # Baseline fantasy points
                            'std': 8.0,    # Baseline uncertainty
                            'confidence_interval': [7.0, 23.0],
                            'percentiles': {
                                'p5': 7.0,
                                'p25': 12.0,
                                'p75': 18.0,
                            ...
                            'team': None,
                            'advanced_stats': {},
                            'note': 'Baseline prediction - no historical data available'
                        }
                else:
                    # Player not found anywhere - create baseline prediction
                    print(f"⚠️  Player {player_name} not found in any data, creating baseline prediction")
                    player_predictions[str(player_name)] = {
                        'mean': 15.0,  # Baseline fantasy points
                        'std': 8.0,    # Baseline uncertainty
                        'confidence_interval': [7.0, 23.0],
                        ...
                        'team': None,
                        'advanced_stats': {},
                        'note': 'Baseline prediction - player not found in any data'
                    }
            
            # Enhanced results with uncertainty quantification AND individual predictions
            results = {
                ...
                # has_weather_data = has_weather_data, # This line is removed
                ...
                # ADVANCED STATS INTEGRATION: Include advanced stats in results
                'advanced_stats_features': {
                    'consistency_score': 'consistency_score' in test.columns,
                    'boom_bust_ratio': 'boom_bust_ratio' in test.columns,
                    'floor_ceiling_spread': 'floor_ceiling_spread' in test.columns,
                    'team_usage_pct': 'team_usage_pct' in test.columns,
                    'home_away_diff': 'home_away_diff' in test.columns,
                    'recent_form': 'recent_form' in test.columns,
                    'season_trend': 'season_trend' in test.columns,
                    'total_advanced_stats_columns': len([col for col in test.columns if any(stat in col for stat in ['consistency', 'boom_bust', 'floor_ceiling', 'team_usage', 'home_away', 'recent_form', 'season_trend', 'position_rank', 'team_dependency', 'early_late', 'opponent_strength', 'qb_efficiency', 'rb_workload', 'wr_big_play', 'te_usage', 'consistency_over_time'])])
                },
                # CRITICAL: Individual player predictions for downstream use
                'player_predictions': player_predictions,
                'predictions_metadata': {
                    'num_players': len(player_predictions),
                    'prediction_method': 'bayesian_hierarchical_unified',
                    'uncertainty_quantified': True,
                    'advanced_stats_integrated': True
                },
            ...
            from ffbayes.utils.path_constants import get_results_dir
            current_year = datetime.now().year
            results_file = str(get_results_dir(current_year) / 'bayesian-hierarchical-results' / 'unified_model_results.json')
        ...
        import sys
        sys.exit(1)  # Exit with error code to signal failure to pipeline
...
def main():
    ...
    try:
        ...
        if trace is not None and results is not None:
            print("\n" + "=" * 60)
            ...
            # Advanced stats features status
            if 'advanced_stats_features' in results:
                print("\nAdvanced Statistics Features:")
                for feature, status in results['advanced_stats_features'].items():
                    if feature == 'total_advanced_stats_columns':
        ...
        print(f"Fatal error in Bayesian analysis: {e}")
        import sys
        sys.exit(1)
