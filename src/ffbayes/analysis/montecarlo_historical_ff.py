...
def find_latest_team_file() -> str:
    ...
    draft_strategy_dir = Path('results/draft_strategy')
    if not draft_strategy_dir.exists():
        return None
    
    # Look for team files with pattern 'team_for_monte_carlo_*.tsv'
    team_files = list(draft_strategy_dir.glob('team_for_monte_carlo_*.tsv'))
...
def make_team(team, db):
    ...
    # Filter database to only include players in our team
    filtered_db = db[db['Name'].isin(my_team_names) & db['Position'].isin(valid_positions)]
    
    if filtered_db.empty:
        print(f"Warning: No players found in database for team: {my_team_names}")
        return pd.DataFrame(columns=['Name', 'Position', 'Tm'])
    
    # Remove duplicates and select only the required columns
    result = filtered_db[['Name', 'Position', 'Tm']].drop_duplicates(subset=['Name', 'Position'])
...
def main(args=None):
    ...
    parser.add_argument("--auto-team-file", action="store_true", help="Automatically find team file from draft strategy output")
    ...
    elif args.auto_team_file:
        # Automatically find team file from draft strategy output
        logger.info("Auto-finding team file from draft strategy output...")
        team_file_path = find_latest_team_file()
        if team_file_path:
            logger.info(f"Found team file: {team_file_path}")
            my_team = interface.handle_errors(load_team_from_file, team_file=team_file_path)
        else:
            interface.log_error("No team file found from draft strategy. Run draft strategy first.", interface.EXIT_DATA_ERROR)
            return
    elif QUICK_TEST:
