...
def get_combined_data(directory_path):
    ...
    try:
        ...
        print(f"   ✅ Available years: {sorted(df['Season'].unique())}")
        return df
...
def find_latest_team_file() -> str:
    """Find the most recent team file from an explicit directory."""
...
def load_team_from_file(team_file: str) -> pd.DataFrame:
    ...
    try:
        ...
        print("🔍 Resolving player names for database matching...")
        ...
        print(f"⚠️  Warning: Name resolution failed: {e}")
...
def make_team(team, db):
    ...
    if missing_players:
        print(f"⚠️  Warning: {len(missing_players)} players not found in historical database:")
    ...
    if filtered_db.empty:
        raise ValueError(
            f"❌ CRITICAL ERROR: No players found in database for team: {my_team_names}\n"
            f"💡 This means the pipeline cannot run Monte Carlo validation.\n"
            f"🔍 Check that player names in your team file match the database format.\n"
            f"📊 Expected format examples: 'Patrick Mahomes' not 'P. Mahomes'"
        )
    ...
    if 'Tm' in result.columns and result['Tm'].isna().any():
        ...
        raise ValueError(
            f"❌ CRITICAL ERROR: Missing team information for players: {missing_teams}\n"
            f"💡 This indicates a data integrity issue in the unified dataset.\n"
            f"🔍 The pipeline cannot proceed without complete player data."
        )
...
def get_score_for_player(db, player, years, max_attempts=100):
    ...
    if player_data.empty:
        print(f'❌ CRITICAL ERROR: Player {player.Name} not found in database')
        raise RuntimeError(
    ...
    if len(available_years) == 0:
        print(f'❌ CRITICAL ERROR: No season data found for {player.Name}')
        raise RuntimeError(
    ...
    if not valid_years:
        print(f'⚠️  Warning: {player.Name} has no data in simulation years {years}')
    ...
    for attempt in range(max_attempts):
        try:
            ...
            if attempt == max_attempts - 1:
                print(f'❌ CRITICAL ERROR: Failed to get score for {player.Name} after {max_attempts} attempts: {e}')
                raise RuntimeError(
    ...
    print(f'❌ CRITICAL ERROR: No valid historical data found for {player.Name}')
    print(f'   Available years: {sorted(available_years)}')
...
def main(args=None):
    ...
    # Load team automatically - use test file for testing, otherwise require an
    # explicit input instead of the legacy my_ff_teams fallback.
    ...
    if args.team_file:
        ...
        if QUICK_TEST:
            ...
            interface.log_error(
                'No team file provided. '
                'Pass --team-file <path> for real roster validation. '
