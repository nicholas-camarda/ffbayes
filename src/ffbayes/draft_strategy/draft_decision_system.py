...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .frontier-tag.primary {
      border-color: rgba(248, 113, 113, 0.38);
      background: rgba(239, 68, 68, 0.12);
      color: #fecaca;
    }
    .frontier-tag.selected {
      border-color: rgba(56, 189, 248, 0.40);
  ...
  <script>
    ...
    (() => {
      ...
      function buildBoardState() {
        ...
        const fallbacks = (recommendedNow.length ? recommendedNow : [...availableRows].sort((a, b) => (Number(b.current_pick_utility) - Number(a.current_pick_utility)) || (Number(b.draft_score) - Number(a.draft_score)))).slice(1, 6).map((row, index) => ({
          ...row,
          recommendation_lane: 'fallback',
          lane_rank: index + 1,
        }));
        const canWait = (recommendedWait.length ? recommendedWait : [...availableRows].sort((a, b) => (Number(b.availability_to_next_pick) - Number(a.availability_to_next_pick)) || (Number(b.draft_score) - Number(a.draft_score)))).filter((row) => !pickNow.some((candidate) => safeLower(candidate.player_name) === safeLower(row.player_name))).slice(0, 5).map((row, index) => ({
          ...row,
      ...
      function renderModelNotes(boardState) {
        ...
        document.getElementById('provenance-panel').innerHTML = `
          <div class="summary-box">${publishProvenance.published_at
            ...
            : 'Publish provenance will appear after `ffbayes pre-draft --stage-pages`, `ffbayes stage-dashboard`, or `ffbayes publish` stages this dashboard.'}</div>
